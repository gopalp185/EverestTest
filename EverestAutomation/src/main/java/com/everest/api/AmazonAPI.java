...
public class AmazonAPI {
	...
	public static void getBookDetails(String bname, String qty, FileWriter fWriter, CSVWriter writef) { 
		...
		try {
			...
			Response response = given()
					...
					.when().get(booklinks.get(i).substring(booklinks.get(i).lastIndexOf(";")+1,booklinks.get(i).length())).then().extract().response();
            ...
            if((docaudio.select("span").text().contains("minute")) || (booklinks.get(i).substring(0, booklinks.get(i).lastIndexOf(";"))).contains(":"))
            {
			...
			for(int c=0;c<prsize;c++)
			{
				...
				{
					String pap_new=((document.select(".a-size-mini").get(c).text().substring(document.select(".a-size-mini").get(c).text().lastIndexOf("from")+4, document.select(".a-size-mini").get(c).text().length()).trim()));	
				    pap_prc=pap_prc+Integer.parseInt(((pap_new.substring(1, pap_new.lastIndexOf(".")).replace(",", "")).trim()));
					flag++;
				}
				else if(document.select(".a-size-mini").get(c).attr("href").contains("hrd_new"))
				{
					String hrd_new=((document.select(".a-size-mini").get(c).text().substring(document.select(".a-size-mini").get(c).text().lastIndexOf("from")+4, document.select(".a-size-mini").get(c).text().length()).trim()));	
				    hrd_prc=hrd_prc+Integer.parseInt(((hrd_new.substring(1, hrd_new.lastIndexOf(".")).replace(",", "")).trim()));
					flag++;
				}
			}
			if(flag==0)
			...
			if(!(bookdetails.contains("Null")))
			{
			FlipkartAPI.getBookDetails(bookdetails,bname,qty,fWriter,writef,count+1);
			}
			bookdetails.clear();
		}
			}
	...
	public static List<String> getBookUrls(String bname) { 
		...
		try {
			...
			Response response = given()
					...
					.header("cp-extension-installed", "Yes")
					.when().get("https://www.amazon.in/s?k="+((bname.replace(" ", "+")).replace("'s", "s"))+"&i=stripbooks&ref=nb_sb_noss").then().extract().response();
			htmlBody = (response.getBody().asString());
			Document document = Jsoup.parse(htmlBody);
			...
			int psize=(documentx.select(".a-size-medium")).size();
				...
				{
					if(((documentx.select(".a-size-medium").get(b).text()).toLowerCase()).startsWith(bname.toLowerCase()) && !(documentx.select(".a-size-medium").get(b).text().contains("+")))
					{
						String bnameNURl=documentx.select(".a-size-medium").get(b).text()+";"+("https://amazon.in"+(documentx.select(".a-size-medium").get(b).parentNode().attr("href")));
