...
public class FlipkartAPI {
	...
	public void getBookDetails(List<String> bookdetails, String bname, String qty, FileWriter fWriter, CSVWriter writef,
			int count) {
		try {
			List<String> fbookdetails = new ArrayList<>();
			boolean amznavailabity;
			boolean fktavailabity;
			Response response = given()
					...
					.header("cp-extension-installed", "Yes").when()
					.get("https://www.flipkart.com/" + bname.replace(" ", "-") + "/p/itmfc9jxsc7dckfm?pid="
							+ (bookdetails.get(4).replace("-", "")).trim())
			...
			if (pbsize > 1) {
				for (int i = 0; i < pbsize; i++) {
					String publisherselectall = documentx.select("._21Ahn-").get(i).text();
					if (publisherselectall.contains("Publisher")) {
						String publisher = (publisherselectall.substring(publisherselectall.lastIndexOf(":") + 1,
								publisherselectall.length())).trim();
						fbookdetails.add(publisher);
					}
				}
			} else {
				fbookdetails.add("Null");
			...
			pc.comparePrices(bookdetails, fbookdetails, Integer.parseInt(qty.trim()));
			String qtyx = "";
			if (Integer.parseInt(bookdetails.get(bookdetails.size() - 1)) > Integer
					.parseInt(fbookdetails.get(fbookdetails.size() - 1))) {
				qtyx = qtyx + bookdetails.get(bookdetails.size() - 1);
			} else if (Integer.parseInt(bookdetails.get(bookdetails.size() - 1)) < Integer
					.parseInt(fbookdetails.get(fbookdetails.size() - 1))) {
			...
			String logs = smpl.logvalues(count, bookdetails.get(4), bname, bookdetails.get(3), bookdetails.get(2), qtyx,
					bookdetails.get(1), amznavailabity, bookdetails.get(bookdetails.size() - 1), fbookdetails.get(1),
					fktavailabity, fbookdetails.get(fbookdetails.size() - 1));
			fWriter.write(logs);
		} catch (Exception e) {
			e.printStackTrace();
		}
